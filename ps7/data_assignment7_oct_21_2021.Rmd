...
```{r}
# outcome variable is favorability of police
before_gf <- filter(protest_df_bydate, before == 1)
after_gf <- filter(protest_df_bydate, before == 0)
mean(before_gf$group_favorability_the_police, na.rm = TRUE)
mean(after_gf$group_favorability_the_police, na.rm = TRUE)

summary(difference_in_means(group_favorability_the_police ~ before, protest_df_bydate))
```
...
```{r}
mean(before_gf$group_favorability_evangelicals, na.rm = TRUE)
mean(after_gf$group_favorability_evangelicals, na.rm = TRUE)

summary(difference_in_means(group_favorability_evangelicals ~ before, protest_df_bydate))
```
...
p <- protest_df_bydate %>% 
  drop_na(group_favorability_the_police, group_fav_white_black) %>%
  dplyr::select(day_running, group_favorability_the_police, group_fav_white_black, before, day) %>%
  group_by(day_running) %>%
  summarise_at(vars(group_favorability_the_police, group_fav_white_black, before), list(avg_police = mean))
```
...
```
```{r}
ggplot(data = p, aes(x=day_running, y = group_favorability_the_police_avg_police, shape = as.factor(before_avg_police))) +
  geom_point() +
...
```{r}
discrimination_df <- protest_df_bydate %>% 
  group_by(day) %>% 
  mutate(avg_group_fav_white_black = mean(discrimination_blacks, na.rm = "TRUE")) %>% 
  dplyr::select(day, before, avg_group_fav_white_black) %>% 
  unique()

ggplot(discrimination_df, aes(day, avg_group_fav_white_black, shape = as.factor(before))) +
  geom_point() +
  geom_smooth(method = "lm", se=FALSE) +
  geom_vline(xintercept = as.Date("2020-05-28"),linetype = "dashed", color = "red")

```

```{r}
p_before <- filter(p, before_avg_police == 1)
p_after <- filter(p, before_avg_police == 0)

m1 <- lm(group_favorability_the_police_avg_police ~ day_running, data=p_before)
m2 <- lm(group_favorability_the_police_avg_police ~ day_running, data=p_after)
...
```{r}
m1 <- lm(group_favorability_the_police ~ pid7 + before, data=protest_df_bydate)
# stargazer(protest_df, type = "text")
# before is zero after the event
summary(m1)
```
...
```{r}
m2 <- lm(group_favorability_the_police ~ pid7 + before + race_ethnicity + group_fav_white_black + female + female * pid7, data=protest_df_bydate)
summary(m2)
...
polr(data = protest_df_bydate, formula = as.factor(group_favorability_the_police) ~ pid7 + before + race_ethnicity + group_fav_white_black + female + female * pid7, method = "probit")
```
