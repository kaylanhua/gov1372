...
I rated my person a 3.5 for Openness to Experience and a 2.5 for conscientiousness. Looking below at the data, I seemed to be right on the nose for O and completely missed the mark on C (they were actually a 4.5 on the C scale).

```{r}
filter(personality_data, `Random ID` == 91567)
```

Meanwhile, I rated my second person a 5 and a 4.5, respectively (Openness and Conscientiousness). I seemed to be very close: they were actually a 4.5 and 5, so the opposite of what I had estimated, but still pretty close. 
```{r}
filter(personality_data, `Random ID` == 65947)
```
...
My partner's guess for the first person were 2 (O) and 3(C). This tells me that the PLSCI is a little less variable than the guesses about O, C, and I. This is because the PLSCI ratings are based primarily on things we can see in the room (e.g. how messy it is, how many CDs there are) versus the guesses, which are purely based on our own inferences. It is important in subject matters like these to rely more on intercoder reliability because the "answers" are much more subjective and there is no ground truth.
...
```{r}
par(mfrow=c(2,1))
hist(personality_data$TIPI_extraversion, breaks=15, col="red")
abline(v = mean(personality_data$TIPI_extraversion, na.rm = T), col = "black", lwd=2)
hist(personality_data$TIPI_agreeableness, breaks=15, col="orange")
abline(v = mean(personality_data$TIPI_agreeableness, na.rm = T), col = "black", lwd=2)
```
```{r}
mean(personality_data$TIPI_extraversion)
mean(personality_data$TIPI_agreeableness)
mean(personality_data$TIPI_conscientiousness)
mean(personality_data$TIPI_emot_stability)
mean(personality_data$TIPI_openness)
```
...
par(mfrow=c(2,1))
hist(personality_data$TIPI_conscientiousness, breaks=15, col="yellow")
abline(v = mean(personality_data$TIPI_conscientiousness, na.rm = T), col = "black", lwd=2)
hist(personality_data$TIPI_emot_stability, breaks=15, col="green")
abline(v = mean(personality_data$TIPI_emot_stability, na.rm = T), col = "black", lwd=2)
```

```{r}
hist(personality_data$TIPI_openness, breaks=15, col="blue")
abline(v = mean(personality_data$TIPI_openness, na.rm = T), col = "black", lwd=2)
```
...
```{r}
labels <- c(names(personality_data))
BFI <- labels[15:24]
TIPI <- labels[25:34]
counter <- 1

for(elt in BFI) {
  a <- personality_data[[elt]]
  b <- personality_data[[TIPI[counter]]]
  res <- cor(a, b)
  print(res)
  counter <- counter + 1
}
...
the number of important factors are the number of triangles above the red dotted lines (FA simulated data, PC simulated data, PC resampled data)—there are 4 of them.
