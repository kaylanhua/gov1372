...
Throughout the assignment, we will provide a running example of how you might approach the tasks. For you own work, please do not use either this example or the George Floyd protests.
...
I would be interested in one of the following events:
1. 2019 El Paso Shooting (August 3) followed immediately by Dayton Shooting (August 4th, 2019)
2. Santa Clarita Shooting (November 14, 2019)
3. Hurricane Dorian (August 24, 2019 – September 10, 2019)
  - https://trends.google.com/trends/explore?date=2019-07-01%202020-01-11&geo=US&q=hurricane 
4. First photo of a black hole (October 22, 2019)
  - https://trends.google.com/trends/explore?date=2019-07-01%202020-01-11&geo=US&q=black%20hole,bill%20gates
  - https://trends.google.com/trends/explore?date=2019-07-01%202020-01-11&geo=US&q=black%20hole,nasa
5. Death of Jeffrey Epstein (August 10, 2019)
  - https://trends.google.com/trends/explore?date=2019-07-01%202020-01-11&geo=US&q=epstein,nasa 
...
```{r}
heat_data1 <- NationscapeVars %>% mutate(across(.cols = everything(), as.logical)) %>% select(c(interest, group_favorability_democrats, group_favorability_republicans, dem_vote_intent, news_sources_facebook, news_sources_cnn, news_sources_msnbc, news_sources_fox, raise_upper_tax, uctaxes2))

plot(t(as.matrix(heat_data1)), col = c('red','green'), las = 2) # we're using the plot() function from base R here; it only works this way because we have installed and loaded the plot.matrix package
```
...
### My answer
My broad question: Was Jeffrey Epstein's death a catalyst for more information seeking, both in terms of polarization and in terms of volume? My hypothesis is that Jeffrey Epstein's death prompted people to look for more information in their echochamber, thereby increasing the need to search for information as well as the motivation to search for more partisan media.
...
- It is ok to do a question even if you think the result might say that there was no significant change. 
...
Gadarian, S.K. and Albertson, B. (2014), Anxiety, Immigration, and the Search for Information. Political Psychology, 35: 133-164. https://doi.org/10.1111/pops.12034

https://academic.oup.com/poq/article-abstract/76/3/576/1894704 
Greg M. Shaw, Laura Gaffey, Trends: American Public Opinion on Economic Inequality, Taxes, and Mobility: 1990–2011, Public Opinion Quarterly, Volume 76, Issue 3, Fall 2012, Pages 576–596, https://doi.org/10.1093/poq/nfs039
...
### My Code
```{r}
Jul18 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190718/ns20190718.dta') %>%
  remove_all_labels() # we need to remove the labels in order for these files to be joined together
Jul25 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190725/ns20190725.dta')%>%
  remove_all_labels()
Aug01 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190801/ns20190801.dta') %>%
  remove_all_labels()
Aug08 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190808/ns20190808.dta')%>%
  remove_all_labels()
Aug15 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190815/ns20190815.dta')%>%
  remove_all_labels()
Aug22 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190822/ns20190822.dta')%>%
  remove_all_labels()
Aug29 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190829/ns20190829.dta')%>%
  remove_all_labels()
Sep05 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190905/ns20190905.dta')%>%
  remove_all_labels()
Sep12 <- read_dta('./Nationscape-DataRelease_WeeklyMaterials_DTA/phase_1_v20200814/ns20190912/ns20190912.dta')%>%
  remove_all_labels()

# join them all together
JulSep2019 <- full_join(Jul18,Jul25) %>% 
  full_join(., Aug01) %>%
  full_join(., Aug08) %>%
  full_join(., Aug15) %>%
  full_join(., Aug22) %>%
...
```{r}
JulSep2019 <- JulSep2019 %>%
  mutate(treated = if_else(start_date > as.Date('2019-08-10'), TRUE, FALSE))
...
difference_in_means(raise_upper_tax ~ treated, data = JulSep2019 %>% filter(as.Date('2019-07-31') < start_date) %>% filter(start_date < as.Date('2019-09-15')))

difference_in_means(uctaxes2 ~ treated, data = JulSep2019 %>% filter(as.Date('2019-07-31') < start_date) %>% filter(start_date < as.Date('2019-09-15')))


cohen.d(raise_upper_tax ~ treated, data = JulSep2019 %>% filter(as.Date('2019-07-31') < start_date) %>% filter(start_date < as.Date('2019-09-15')))

cohen.d(uctaxes2 ~ treated, data = JulSep2019 %>% filter(as.Date('2019-07-31') < start_date) %>% filter(start_date < as.Date('2019-09-15')))
```
...
```{r}
# mod_dat <- oscars2020 %>% 
#   mutate(asian = ifelse(race_ethnicity > 3 & race_ethnicity < 11, 1, 0)) %>% 
#   filter(as.Date('2020-02-04') < start_date) %>%
#   filter(start_date < as.Date('2020-02-14'))

# favor_mod <- lm(group_favorability_asians ~ treated + asian + education + household_income, mod_dat)

tax_plot <- JulSep2019 %>% 
  filter(as.Date('2019-07-31') < start_date) %>% filter(start_date < as.Date('2019-09-09')) %>%
  mutate(start_date = ymd(substr(start_date, 1, 10))) %>%
  group_by(start_date, treated) %>% 
  summarize(day_taxes = mean(uctaxes2, na.rm = T)) %>% 
  ggplot(aes(x = start_date, y = day_taxes, color = treated, group = treated)) +
  geom_point() +
  geom_smooth(method = "lm", se = F) +
  geom_vline(xintercept = ymd("2020/02/09"), lty = 2, size = 1.2) +
  labs(x = "Survey Date", y = "Date vs. Taxation on Rich") + 
  theme_gray()
